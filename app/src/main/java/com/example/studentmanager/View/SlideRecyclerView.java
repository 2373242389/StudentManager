...
public class SlideRecyclerView extends RecyclerView {
    ...
    @Override
    public boolean onInterceptTouchEvent(MotionEvent e) {//拦截
        int x = (int) e.getX();
        int y = (int) e.getY();
        obtainVelocity(e);
        switch (e.getAction()) {
    ...
    @Override
    public boolean onTouchEvent(MotionEvent e) { //如果onInterceptTouchEvent返回true，后续事件传递给OnTouchEvent
    ...
    private void releaseVelocity() { //释放
    ...
    private void obtainVelocity(MotionEvent event) {//获取
        if (mVelocityTracker == null) {
    ...
    public int pointToPosition(int x, int y) {//位置
